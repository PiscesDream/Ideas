...
if __name__ == '__main__':
    ...
    if test_raw:
        ...
        print cl.prob(theano.shared(x))
        print cl.pred(theano.shared(x))

        x = asarray(random.binomial(1, 0.3, size=(1, 1, 28, 28)), dtype = theano.config.floatX)
        print cl.prob(x)[0]
        import matplotlib.pyplot as plt

        x = asarray(random.binomial(1, 0.3, size=(1, 1, 28, 28)), dtype = theano.config.floatX)
        plt.imshow(x.reshape(28, 28), cmap='PuBu', interpolation='None', alpha = 0.5)
        x = asarray(random.binomial(1, 0.3, size=(1, 1, 28, 28)), dtype = theano.config.floatX)
        plt.imshow(x.reshape(28, 28), cmap='PuRd', interpolation='None', alpha = 0.5)
        plt.show();
