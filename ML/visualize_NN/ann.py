...
class ANN(object):
    ...
    def fit(self, datasets, batch_size = 500, n_epochs = 200, lr = 0.01, visualize = False):
        ''' without validation'''
...
#        print numpy.mean([debug_f(i) for i in xrange(n_test_batches)]) 
    ...
    def visualize(self, start=1):
        ...
        plt.clf()
        curleft = 0
        curtop = map(lambda x: node_margin*(x+1)/2.0, self.hiddens[start:])
        for ind, ele in enumerate(self.hid_layers[start:]):
            w = numpy.concatenate([ele.W.eval(), ele.b.eval().reshape(1, -1)])

            for i in range(w.shape[0]):
                y = curtop[ind] - i*node_margin
                plt.plot([curleft], [y], 'ro') 
            for i in range(w.shape[1]):
                y = curtop[ind+1] - i*node_margin
                plt.plot([curleft+layer_margin], [y], 'ro')  
            for i in range(w.shape[0]):
                for j in range(w.shape[1]):
                    if w[i, j] > 0:
#                        w = numpy.exp(w)
                        plt.plot([curleft, curleft+layer_margin], 
                                 [curtop[ind]-i*node_margin, curtop[ind+1]-j*node_margin], '-', color='blue', lw=w[i, j]/w.max()) 
                    else:
#                        w = numpy.exp(w)
                        plt.plot([curleft, curleft+layer_margin], 
                                 [curtop[ind]-i*node_margin, curtop[ind+1]-j*node_margin], '-', color='black' , lw=w[i, j]/w.max()) 


            curleft += layer_margin
...
if __name__ == '__main__':
    ...
    cl = ANN(28 * 28, 10, hiddens = [10, 4])
    print cl
    cl.fit(datasets, lr = 0.01, visualize = 1, n_epochs = 500)
