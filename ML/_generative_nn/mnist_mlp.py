...
#        print numpy.mean([debug_f(i) for i in xrange(n_test_batches)]) 
    ...
    def train_x(self):
        ...
        x = theano.shared( numpy.random.uniform(0, 1, size=(1, 1, 28, 28)) )

        lr = 0.01
        index = 9
        real_loss = self.hid_layers[-1].output[0, index] 
        loss = -T.log(self.hid_layers[-1].output[0, index])
...
#        k = cPickle.load(open('classifier/mnist_cnn_00114.dat', 'rb'))
    ...
    if simple_cnn:
        k = cnn(dim_in = 1, size_in = (28, 28), size_out = 10, 
               nkerns = [(10, (3, 3), (2, 2))], 
               nhiddens=[30])
        k.fit(datasets, n_epochs = 5, batch_size = 100, learning_rate = 0.01)
