...
#       topx = 3
#       topx_pred = T.sort(hid_layers[-1].output, 1)[:, :topx]
#       topx_error = T.mean(T.any(T.eq(y.reshape(x.shape[0], 1), topx_pred), 1), 1)
    ...
    def fit(self, datasets, batch_size = 500, n_epochs = 200, lr = 0.01):
        ...
        test_model_on_train = theano.function([], self.errors,
            givens = {
                self.x: train_set_x,
                self.y: train_set_y})
...
#        print numpy.mean([debug_f(i) for i in xrange(n_test_batches)]) 
        ...
        while iteration < maxiter:
            ...
            print ''
            print 'error on test = %f' % test_model()
            print 'error on train = %f' % test_model_on_train()
